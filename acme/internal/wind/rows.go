...
func RowInit(row *Row, r draw.Rectangle) {
	adraw.Display.ScreenImage.Draw(r, adraw.BlackDefault, nil, draw.ZP)
	row.R = r
	row.Col = nil
	r1 := r
	...
	adraw.Display.ScreenImage.Draw(r1, adraw.BlackDefault, nil, draw.ZP)
...
func Rowresize(row *Row, r draw.Rectangle) {
	...
	for i := 0; i < len(row.Col); i++ {
		...
		if i > 0 {
			...
			adraw.Display.ScreenImage.Draw(r2, adraw.BlackDefault, nil, draw.ZP)
			r1.Min.X = r2.Max.X
		}
		Colresize(c, r1)
	}
